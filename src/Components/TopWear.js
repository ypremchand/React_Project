...
export default function Topwear() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://api.apify.com/v2/datasets/L3yyMuEL1hlKTSfVp/items?clean=true&format=json');
        if (!response.ok) {
  ...
  return (
    <div>
      <div className="container">
        <h1 className="my-4">Topwear</h1>
        <div className="row">
          {products.length > 0 ? (
            products.map((product) => (
              <div key={product.id} className="col-md-6 col-lg-4 mb-4">
                <div className="card h-100" style={{ backgroundColor: "#e0ae75" }}>
                  <img
