...
export default function FootWear() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://api.apify.com/v2/datasets/XEg1o1WxCoY83dw5R/items?clean=true&format=json');
        if (!response.ok) {
  ...
  return (
    <div>
      ...
      <h1 className="my-2">FootWear...</h1>
      <div className="row">
        {products && products.length > 0 ? (
          products.map((product) => (
            <div key={product.id} className="col-md-4 mb-4">
              <div className="card h-100" style={{backgroundColor:"#40de94"}}>
                <img 
