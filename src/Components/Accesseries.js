...
export default function Accessories() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://api.apify.com/v2/datasets/oK2r5CNmWw4jWbOYF/items?clean=true&format=json');
        if (!response.ok) {
  ...
  return (
    <div>
      ...
      <h1 className="my-2">Accessories...</h1>
      <div className="row">
        {products && products.length > 0 ? (
          products.map((product) => (
            <div key={product.id} className="col-md-4 mb-4">
              <div className="card h-100 text-white" style={{backgroundColor:"#913906"}}>
                <img 
